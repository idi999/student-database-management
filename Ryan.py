...
def main():
    ...
    student_database = load_from_file("students.txt")
    ...
    while True:
        ...
        if choice == '1':
            ...
            add_student(student_database, name, age, grade)
            ...
            student = search_student(student_database, name)
            if student:
            ...
            update_grade(student_database, name, new_grade)
            print(f"Grade updated for student {name}.")
        elif choice == '5':
            name = input("Enter the name of the student to delete: ")
            delete_student(student_database, name)
            print(f"Student {name} deleted successfully.")
        elif choice == '6':
            count = count_students(student_database)
            print(f"There are {count} students in the database.")
        elif choice == '7':
            avg_grade = average_grade(student_database)
            print(f"The average grade is: {avg_grade:.2f}")
        elif choice == '8':
            save_to_file(student_database, "students.txt")
